(() => {
  ...
  async function loadOpportunities() {
    ...
    const rows=data||[];
    holder.innerHTML=rows.length?rows.map(r=>{const o=r.opportunities||{};return `<div class="list-card"><div class="list-card-top"><div><h3>${esc(o.title||o.topic)}</h3><div class="meta"><span>${esc(o.training_type)}</span><span>${esc(o.category)}</span><span>${esc(o.location||'TBC')}</span><span>${fmtDate(o.training_date)}</span></div></div>${statusBadge(o.status)}</div><p class="muted">Target: ${esc(o.target_audience||'-')} · ${esc(o.duration||'-')} · ${esc(o.estimated_pax||'-')} pax</p><p class="muted">Response deadline: <strong>${fmtDateTime(o.response_deadline)}</strong></p><div class="btn-row"><button class="btn btn-primary" data-interest="${o.id}">I'm Interested</button><button class="btn btn-outline" data-unavailable="${o.id}">Not Available</button><button class="btn btn-soft" data-decline="${o.id}">Not Interested</button></div></div>`}).join(''):'<div class="empty">No matching opportunities at the moment.</div>';
