(() => {
  ...
  async function loadTrainers(){
    ...
    const {data,error}=await client.from('profiles').select('*,trainer_onboarding(*),trainer_preferences(categories)').order('created_at',{ascending:false}).limit(300);
    ...
    const render=filter=>{const q=String(filter||'').trim().toLowerCase();const filtered=trainers.filter(t=>{const pref=Array.isArray(t.trainer_preferences)?t.trainer_preferences[0]:t.trainer_preferences;return !q||[t.full_name,t.email,t.state,...(pref?.categories||[])].join(' ').toLowerCase().includes(q)});holder.innerHTML=filtered.length?filtered.map(t=>{const o=Array.isArray(t.trainer_onboarding)?t.trainer_onboarding[0]:t.trainer_onboarding;const complete=Boolean(o?.onboarding_completed_at);const pref=Array.isArray(t.trainer_preferences)?t.trainer_preferences[0]:t.trainer_preferences;const categories=pref?.categories||[];return `<details class="list-card collapsible-card"><summary><strong>${esc(t.full_name)}</strong><span>${badge(t.collaboration_status)}</span></summary><div class="card-details"><div class="meta"><span>${esc(t.email)}</span><span>${esc(t.phone)}</span><span>${esc(t.state)}</span></div><div class="tag-wrap">${categories.length?categories.map(c=>`<span class="tag">${esc(c)}</span>`).join(''):'<span class="muted">No approved category</span>'}</div><p class="muted">${esc(t.expertise_summary)}</p><div class="meta"><span>Onboarding: <strong>${complete?'Complete':'Pending'}</strong></span><span>Availability: ${esc(t.availability_status)}</span><span>Joined: ${fmt(t.created_at)}</span></div>${o?.profile_photo_url?`<div class="admin-photo"><img src="${esc(o.profile_photo_url)}" alt="${esc(t.full_name)}"></div>`:''}<div class="btn-row">${complete&&t.collaboration_status!=='ACTIVE'?`<button class="btn btn-primary" data-activate="${t.id}">Activate Trainer</button>`:''}${t.collaboration_status==='ACTIVE'?`<button class="btn btn-outline" data-inactivate="${t.id}">Set Inactive</button>`:''}</div></div></details>`}).join(''):'<div class="empty">No matching trainers.</div>';holder.querySelectorAll('[data-activate]').forEach(b=>b.addEventListener('click',()=>setTrainerStatus(b.dataset.activate,'ACTIVE')));holder.querySelectorAll('[data-inactivate]').forEach(b=>b.addEventListener('click',()=>setTrainerStatus(b.dataset.inactivate,'INACTIVE')));};render('');$('trainerSearch')?.addEventListener('input',e=>render(e.target.value));
  }
