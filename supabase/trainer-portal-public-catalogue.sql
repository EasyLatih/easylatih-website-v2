...
grant select (id,title,category,training_type,programme_overview,learning_outcomes,duration,delivery_method,modules,trainer_id,target_participants,publish_status,etris_status) on public.programmes to anon;
...
grant select (id,full_name,professional_bio,collaboration_status) on public.profiles to anon;
...
grant select (trainer_id,profile_photo_url,photo_consent_at) on public.trainer_onboarding to anon;
